...
export async function GET(req: Request) {
  ...
  try {
    ...
    const mlApiUrl = `http://127.0.0.1:8000/old_user_recommendations?user_id=${encodeURIComponent(
      userId
    ...
    await Products.deleteMany({ user_id: userId, k });

    // Insert new ones
    await Products.insertMany(productsToSave);
